...
{
    //los datos entre parentisis son los reales de la bbdd
   // prepare: Este método prepara una sentencia SQL para su ejecución y devuelve un objeto de declaración.
    ...
    //bindParam(":nombre", $nombre);: Este método vincula el marcador de posición :nombre en la sentencia
    // SQL con el valor de la variable $nombre(el nombre de la bbdd, el nombre real de la columna)
}
...
{
    $consulta = $conexion->prepare("DELETE d FROM donantes d LEFT JOIN historic o h ON d.id = h.idDonante where d.id =$idDonante");
    return $consulta->execute();
}
