...
$conexion = get_conexion(); // establecemos conexión con la bbdd
seleccionar_bd_donacion($conexion);

$mensajes = array();  // creamos e inicializamos un array para guardar mensajes de error y de éxito

$nombre = "";   // declaramos las variables
$apellidos = "";
$grupoSanguineo = "";
$edad = "";
$codPostal = "";
...
// isset: es una función en PHP que se utiliza para verificar si una variable está definida y no es NULL
if (isset($_POST['submit'])) {  // verificamos si el formulario se ha enviado y por el método Post
    if (!empty($_POST['name'])) {
    ...
    // si todos los datos han sido validados llamamos a la función para insertar en la bbdd
    //los nombres de las variables se llaman igual que en la tabla
    if (count($mensajes) ==  0) {
...
//Se utiliza la función get_mensajes_html_format para convertir los mensajes del array $mensajes
// en formato HTML y se muestran en la página.
?>
